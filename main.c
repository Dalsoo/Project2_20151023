...
app_timer_id_t m_Main_timer_id;
app_timer_id_t m_Adc_timer_id;
app_timer_id_t m_Ble_Tx_Timer_id;
app_timer_id_t m_Ble_eeprom_read_timer_id;
app_timer_id_t m_Hrs_data_read_timer_id;

app_timer_id_t	m_Ble_Led_Timer_id;
...
void nus_data_handler(ble_nus_t * p_nus, uint8_t * p_data, uint16_t length)   //jsu    bluetooth RX
{
	 sys_BleRxStruct *pBleRx = &sys_BleRx_str;
	
	 pBleRx->BleRxLength = length;
    for (int i = 0; i < length; i++)
    {
	  sys_ble_rx_buf[i] = p_data[i];
    }
		pBleRx->BleRxEndFlag=true;
...
void UART0_IRQHandler(void)
{
    ...
    {
        err_code = ble_nus_send_string(&m_nus, data_array, index + 1);    //jsu ble tx
